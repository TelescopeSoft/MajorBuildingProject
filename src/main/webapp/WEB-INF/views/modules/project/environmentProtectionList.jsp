...
<body>
	...
	<form:form id="searchForm" modelAttribute="environmentProtection" action="${ctx}/project/environmentProtection/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			<li><label style="width:150px;">发改项目代码：</label>
				<form:input path="prjCode" htmlEscape="false" maxlength="64" class="input-medium"/>
			</li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="environmentProtection">
			<tr>
				<td><a href="${ctx}/project/environmentProtection/form?id=${environmentProtection.id}">
					${environmentProtection.prjCode}
				</a></td>
				<td>
					${fns:getDictLabel(environmentProtection.closeFlag, 'environment_close', '是')}
				</td>
				<td>
					${fns:getDictLabel(environmentProtection.washFlag, 'environment_close', '是')}
				</td>
				<!--<td>
				...
				<shiro:hasPermission name="project:environmentProtection:edit"><td>
    				<a href="${ctx}/project/environmentProtection/form?id=${environmentProtection.id}">修改</a>
					<a href="${ctx}/project/environmentProtection/delete?id=${environmentProtection.id}" onclick="return confirmx('确认要删除该施工期环境保护措施吗？', this.href)">删除</a>
