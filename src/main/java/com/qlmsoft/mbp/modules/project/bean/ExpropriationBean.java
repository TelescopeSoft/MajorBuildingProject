/**
 * Copyright &copy; 2017-2018 千里目软件 All rights reserved.
 */
package com.qlmsoft.mbp.modules.project.bean;

import java.io.Serializable;

/**
 * Created by aaronhuang on 2018/7/24.
 */
public class ExpropriationBean implements Serializable {
    private static final long serialVersionUID = 1L;

    private String cz;
    private String fbdw;

    private String ggssc;
    private String ggssz;

    private String lcid;
    private String pfwh;
    private String pzsj;
    private String xmmc;
    private String xmwzmc;
    private String xzqhdm;
    private String zfstate;
    private String status;


    public String getCz() {
        return cz;
    }

    public void setCz(String cz) {
        this.cz = cz;
    }

    public String getFbdw() {
        return fbdw;
    }

    public void setFbdw(String fbdw) {
        this.fbdw = fbdw;
    }

    public String getGgssc() {
        return ggssc;
    }

    public void setGgssc(String ggssc) {
        this.ggssc = ggssc;
    }

    public String getGgssz() {
        return ggssz;
    }

    public void setGgssz(String ggssz) {
        this.ggssz = ggssz;
    }

    public String getLcid() {
        return lcid;
    }

    public void setLcid(String lcid) {
        this.lcid = lcid;
    }

    public String getPfwh() {
        return pfwh;
    }

    public void setPfwh(String pfwh) {
        this.pfwh = pfwh;
    }

    public String getPzsj() {
        return pzsj;
    }

    public void setPzsj(String pzsj) {
        this.pzsj = pzsj;
    }

    public String getXmmc() {
        return xmmc;
    }

    public void setXmmc(String xmmc) {
        this.xmmc = xmmc;
    }

    public String getXmwzmc() {
        return xmwzmc;
    }

    public void setXmwzmc(String xmwzmc) {
        this.xmwzmc = xmwzmc;
    }

    public String getXzqhdm() {
        return xzqhdm;
    }

    public void setXzqhdm(String xzqhdm) {
        this.xzqhdm = xzqhdm;
    }

    public String getZfstate() {
        return zfstate;
    }

    public void setZfstate(String zfstate) {
        this.zfstate = zfstate;
    }

    public String getStatus() {
        return status;
    }

    public void setStatus(String status) {
        this.status = status;
    }


}
